...
class SettingsViewModel(
    ...
    fun updatePlusRange(plusRange: ClosedFloatingPointRange<Float>) {
        _settingsState.value = _settingsState.value.copy(plusRange = plusRange.start to plusRange.endInclusive)
        saveState()
    }

    fun updateMinusRange(minusRange: ClosedFloatingPointRange<Float>) {
        _settingsState.value = _settingsState.value.copy(minusRange = minusRange.start to minusRange.endInclusive)
        saveState()
    }

    fun updateMultiplyRange(multiplyRange: ClosedFloatingPointRange<Float>) {
        _settingsState.value = _settingsState.value.copy(multiplyRange = multiplyRange.start to multiplyRange.endInclusive)
        saveState()
    }

    fun updateDivideRange(divideRange: ClosedFloatingPointRange<Float>) {
        _settingsState.value = _settingsState.value.copy(divideRange = divideRange.start to divideRange.endInclusive)
        saveState()
    }
